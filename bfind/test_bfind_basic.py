...
def test_links(tmpdir):
    """Tests for the -links filter."""
    print(f"\n{BOLD}--- -links filter ---{RESET}")

    tree = os.path.join(tmpdir, "links")
    os.makedirs(tree)

    # A regular file starts with exactly one hard link
    solo = os.path.join(tree, "solo.txt")
    open(solo, "w").close()

    # Create a file and a hard link to it (both entries report nlink == 2)
    original = os.path.join(tree, "original.txt")
    open(original, "w").close()
    linked = os.path.join(tree, "linked.txt")
    os.link(original, linked)

    rc, lines, _ = run_bfind([tree, "-links", "1", "-type", "f"])
    found = set(lines)
    check("-links 1 finds only files with one hard link",
          rc == 0 and solo in found
          and original not in found and linked not in found,
          f"Got: {found}")

    rc, lines, _ = run_bfind([tree, "-links", "2", "-type", "f"])
    found = set(lines)
    check("-links 2 finds both entries sharing the same inode",
          rc == 0 and original in found and linked in found
          and solo not in found,
          f"Got: {found}")


def test_samefile(tmpdir):
    """Tests for the -samefile filter."""
    print(f"\n{BOLD}--- -samefile filter ---{RESET}")

    tree = os.path.join(tmpdir, "samefile")
    os.makedirs(tree)
    ...
    other = os.path.join(tree, "other.txt")
    open(other, "w").close()

    # -samefile original should match original and linked (same inode)
    rc, lines, _ = run_bfind([tree, "-samefile", original])
    found = set(lines)
    check("-samefile finds the reference file and its hard links",
          rc == 0 and original in found and linked in found
          and other not in found,
          f"Got: {found}")

    # -samefile on a file with no hard links finds only itself
    rc, lines, _ = run_bfind([tree, "-samefile", other])
    found = set(lines)
    check("-samefile with no hard links finds only the file itself",
          rc == 0 and other in found
          and original not in found and linked not in found,
          f"Got: {found}")

    # A symlink to original should NOT match -samefile without -L
    # (lstat gives the symlink its own inode, distinct from the target's)
    sym = os.path.join(tree, "sym_to_original")
    os.symlink("original.txt", sym)
    rc, lines, _ = run_bfind([tree, "-samefile", original])
    found = set(lines)
    check("-samefile: symlink to target does not match without -L",
          rc == 0 and original in found and sym not in found,
          f"Got: {found}")
...
def main():
    ...
    print(f"  these does not guarantee full scores. Test edge cases")
